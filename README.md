...
- `?timeMode=true-solar`：真太陽時四柱正式使用頁面上方查詢時間；飛星、金函玉鏡與奇門遁甲仍維持手錶時間。

頁面上方的日期時間正式稱為「排盤時間」，可跟隨現在時間或手動查詢。手動修改排盤時間會暫停現在時間自動更新；點「現在時間」可恢復持續更新。真太陽時頁面的裝置／自訂時間僅作獨立換算查詢，不改變正式排盤時間。
...
不再使用單頁「套用真太陽時至全部排盤」操作。真太陽時四柱共用頁面上方排盤時間（臺灣 UTC+8）：以 civil instant 判斷年／月節氣，並使用 true-solar local parts 判斷日／時；飛星、金函玉鏡與奇門遁甲仍維持手錶時間。真太陽時區塊的裝置／自訂時間仍可獨立換算，不會改動正式四柱。座標、定位與計算都在瀏覽器本地進行，不呼叫外部 API，也不使用 localStorage。
