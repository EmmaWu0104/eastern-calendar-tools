...
- `?timeMode=true-solar`：真太陽時四柱、九宮飛星與金函玉鏡日盤正式使用頁面上方查詢時間；登貴與奇門遁甲仍維持既有手錶時間。
...
不再使用單頁「套用真太陽時至全部排盤」操作。真太陽時四柱、九宮飛星與金函玉鏡日盤共用頁面上方排盤時間（臺灣 UTC+8）：四柱以 civil instant 判斷年／月節氣，並使用 true-solar local parts 判斷日／時；九宮飛星與金函玉鏡沿用各自同一份正式 ChartTimeContext 結果。登貴與奇門遁甲仍維持既有手錶時間基準。真太陽時區塊的裝置／自訂時間仍可獨立換算，不會改動正式排盤。座標、定位與計算都在瀏覽器本地進行，不呼叫外部 API，也不使用 localStorage。
